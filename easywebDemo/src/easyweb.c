...
void screen1 (void)
{
	...
	oled_putString(1,1,  (uint8_t*)"Easy Web Server", OLED_COLOR_BLACK, OLED_COLOR_WHITE);
...
void screen2 (void)
{
	...
	oled_putString(1,1,  (uint8_t*)"EC020 - LPC", OLED_COLOR_BLACK, OLED_COLOR_WHITE);
	oled_putString(1,17, (uint8_t*)"Light:", OLED_COLOR_BLACK, OLED_COLOR_WHITE);

	intToString(GetAD7Val(), buf, 10, 10);
	oled_fillRect((1+9*6),17, 80, 24, OLED_COLOR_WHITE);
	oled_putString((1+9*6),17, buf, OLED_COLOR_BLACK, OLED_COLOR_WHITE);

	oled_putString(1,25, (uint8_t*)"			", OLED_COLOR_BLACK, OLED_COLOR_WHITE);
...
int main (void)
{
  ...
  while (1)                                      // repeat forever
  {
	...
	if(aaPagecounter%4 == 0 && flag == 0){
		oled_clearScreen(OLED_COLOR_WHITE);
		screen1();
		flag = 1;
	}
	if(aaPagecounter%4 == 2 && flag == 1){
		oled_clearScreen(OLED_COLOR_WHITE);
		screen2();
		flag = 0;
	}
     ...
     if (*(Key + 1) == 'D')
       if (*(Key + 3) == '%')
         switch (*(Key + 2))
         {
           ...
           case '7' :                                 // "AD7%"?
           {
             sprintf(NewKey, "%3u", GetAD7Val());     // copy saved value from previous read
             memcpy(Key, NewKey, 3);                 
             break;
           }
		   case '1' :                                 // "AD1%"?
     ...
     if (*(Key + 1) == 'D')
       if (*(Key + 3) == '%')
         switch (*(Key + 2))
         {
           ...
           case 'A' :                                 // "ADA%"?
           {
             //sprintf(NewKey, "%3u", 20);    // insert AD converter value
